			...
			>
				<div className="flex flex-row justify-between items-center w-full rtl ">
					<div className="flex w-full justify-between items-center text-sm text-gray-700 px-7 text-center">
						...
						<span
							className={`w-[16%] flex justify-center ${getStatusColor()}`}
						>
							{status}
						</span>
						<span className="w-[16%] flex justify-center">
            ...
            /> */}
					...
					<DialogFooter className="flex flex-row justify-center items-center rounded-l self-center mr-auto">
						...
						<button
							onClick={() => {
								const text =
									panelDetailRef.current?.innerText || "";
								navigator.share({
									title: "جزئیات پنل",
									text,
								});
							}}
							className="flex items-center gap-1 border-1 rounded px-2 py-1 text-sm text-muted-foreground hover:text-primary cursor-pointer"
						>
						...
						<Formik
							...
							onSubmit={(values) => {
								...
								const body = {
									...
									totalNumberOfModules: typeCasting(
										values.totalNumberOfModules
									),
