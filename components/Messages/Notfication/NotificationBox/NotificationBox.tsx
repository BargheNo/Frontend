	...
	return (
		<>
			<Dialog open={open} onOpenChange={setOpen}>
				...
				<DialogContent className="w-full sm:min-w-[500px] max-w-l p-4 overflow-auto py-4 bg-[#F1F4FC]">
					...
					<div className="flex flex-col  text-gray-700 mt-2 items-end self-end">
						{typeid === 2 && (
							<>
								...
								<NotificationsDialogFields
									title=" هزینه نصب : "
									detail={toFarsiNumber(
										notificationContent?.data?.cost
									)}
								/>
								<NotificationsDialogFields
						...
						{typeid === 3 && (
							<>
								<NotificationsDialogFields
									title=" نام پنل: "
									detail={
										notificationContent?.data?.panel?.name
									}
								/>
								<NotificationsDialogFields
									title=" نام مشتری: "
									detail={
										notificationContent?.data?.panel
											?.customer?.firstName +
										" " +
										notificationContent?.data?.panel
											?.customer?.lastName
								...
								<NotificationsDialogFields
									title="  وضعیت نصب: "
									detail={notificationContent?.data?.status}
								/>
							</>
						)}
						{typeid === 4 && (
							<>
								...
								<NotificationsDialogFields
									...
									detail={toFarsiNumber(
										notificationContent.data
											.maintenanceRecord.customer.phone
									)}
								/>
								<NotificationsDialogFields
									title=" نام اوپراتور: "
								...
								<NotificationsDialogFields
									...
									detail={
										notificationContent.data
											.maintenanceRecord.corporation.name
									}
								/>
								<NotificationsDialogFields
									title="   نام پنل: "
							...
							<div className="flex flex-col lg:justify-center justify-end lg:mb-0 -mb-20 gap-2 items-center md:mb-10 md:mr-0 -mr-40 min-w-48 ">
								<HoverCard>
									<HoverCardTrigger asChild>
										<Button
											className="cursor-pointer mr-auto mb-auto -mt-2 -ml-2"
											variant="link"
										>
											<EllipsisVertical />
										</Button>
								...
								<div className="flex flex-col md:ml-10 ml-23 items-center justify-center gap-3 p-5 md:mt-0 mt-15">
									...
									<div
										className={`cta-neu-button flex bg-white items-center md:w-auto w-40 content-center justify-center `}
									>
										<button
											onClick={() => setOpen(!open)}
											className="cursor-pointer whitespace-nowrap text-[0.9rem]"
										>
											مشاهده جزئیات
										</button>
					...
					<ContextMenuItem
						...
						onClick={() => {
							notificationContent.isRead === false
								? notificationService
										.markAsRead(notificationContent.id)
										.then((res) => setread("خوانده شده"))
								: toast.success("قبلا خوانده شده.");
						}}
					>
						خوانده شد
					</ContextMenuItem>
