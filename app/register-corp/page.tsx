...
import { setRegisterCorpId } from "@/src/store/slices/corpSlice";
...
const validationSchemaForm = Yup.object({
    ...
    nationalID: Yup.string()
        ...
        .length(11, "شناسه ملی باید 11 رقم باشد"),
    iban: Yup.string()
        .required("شماره شبا الزامی است")
        .length(26, "شماره شبا باید 26 رقم باشد"),
        ...
        if (step === 0) {
            ...
            if (corpId) {
                        ...
                        if (
                            ...
                            if (signatoriesOk) {
                                if (Object.keys(formData).length !== 0) {
                                    putData({
                                        endPoint: `/v1/user/corps/registration/${corpId}/basic`,
                                        data: formData,
                                    })
                                        .then((res) => {
                                            console.log("res", res);
                                ...
                                CustomToast(
                                    "وارد کردن حداقل یک صاحب امضا الزامی است.",
                                    "error"
                                );
                            }
                        }
                ...
                if (
                    ...
                    if (signatoriesOk) {
                        postData({
                            endPoint: `/v1/user/corps/registration/basic`,
                            data: {
                                name: values.name,
                            ...
                            .then((res) => {
                                ...
                                dispatch(setRegisterCorpId(res?.data?.id));
                                CustomToast(res?.message, "success");
                                if (step < steps.length - 1) {
                                    setStep(step + 1);
                                }
                    ...
                    .then(async (res) => {
                        ...
                        if (contactInformationOk) {
                            if (formData?.contactInformation?.length !== 0) {
                                postData({
                                    endPoint: `/v1/user/corps/registration/${corpId}/contacts`,
                                    data: formData,
                                })
                                    .then((res) => {
                                        console.log("res", res);
                    ...
                    .then(async (res) => {
                        ...
                        if (addressesOk) {
                            if (formData?.addresses?.length !== 0) {
                                postData({
                                    endPoint: `/v1/user/corps/registration/${corpId}/address`,
                                    data: {
            ...
            if (corpId) {
                ...
                setLoading(true);
                putDataFile({
                    endPoint: `/v1/user/corps/registration/${corpId}/certificates`,
                    formData: formData,
                })
                    .then((res) => {
